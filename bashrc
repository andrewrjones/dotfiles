...
cyan="\033[1;36m";
if [ "$PS1" ]; then
  PROMPT_COMMAND='PS1="\e[37m[\[\$(date +%H:%M:%S)\]]\\[\033[0;33m\][\!]\`if [[ \$? = "0" ]]; then echo "\\[\\033[32m\\]"; else echo "\\[\\033[31m\\]"; fi\`[\u@\h: \`if [[ `pwd|wc -c|tr -d " "` > 18 ]]; then echo "\\W"; else echo "\\w"; fi\`]\n\$\[\033[0m\] "; echo -ne "\033]0;\H:`pwd`\007"'
fi
